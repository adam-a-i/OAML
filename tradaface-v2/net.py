...
import inspect
import traceback
# import transmatcher # <-- Moved to fix circular import
...
class Backbone(Module):
    def __init__(self, input_size, num_layers, mode='ir'):
        ...
        if input_size[0] == 112:
            ...
            # Add TransMatcher as a separate component (like QAConv in root net.py)
            from transmatcher import TransMatcher
            self.transmatcher = TransMatcher(seq_len=49, d_model=512, num_decoder_layers=3, dim_feedforward=2048)
        else:
            self.output_layer = Sequential(
                BatchNorm2d(output_channel), Dropout(0.4), Flatten(),
                Linear(output_channel * 14 * 14, 512),
                BatchNorm1d(512, affine=False))
            # Add TransMatcher for 224x224 input
            from transmatcher import TransMatcher
            self.transmatcher = TransMatcher(seq_len=196, d_model=512, num_decoder_layers=3, dim_feedforward=2048)
    ...
    def forward(self, x, return_feature_map=False):
        try:
            ...
            # Final normalization of the feature map before it's returned
            feature_map_flat = feature_map.view(feature_map.size(0), -1)
            feature_map_norm = torch.norm(feature_map_flat, p=2, dim=1, keepdim=True)
            feature_map_norm = torch.clamp(feature_map_norm, min=1e-8) # prevent division by zero
            feature_map_flat = feature_map_flat / feature_map_norm
            feature_map = feature_map_flat.view_as(feature_map)

            # Output layer
            x = self.output_layer(x)
            
            norm = torch.norm(x, 2, 1, True)
            output = torch.div(x, norm + 1e-6)
            
            if return_feature_map:
                return output, norm, feature_map, self.transmatcher
            else:
                return output, norm, self.transmatcher
                
        except Exception as e:
            print(f"[BACKBONE_FORWARD] ERROR in backbone forward pass: {e}")
