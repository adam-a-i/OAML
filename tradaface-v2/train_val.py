...
class Trainer(pl.LightningModule):
    def __init__(self, **kwargs):
        ...
        # Use the original pairwise loss that worked with QAConv
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.pairwise_matching_loss is not None:
            try:
                ...
                # Compute TransMatcher loss using the original pairwise loss
        ...
        total_loss = adaface_loss + 0.01 * transmatcher_loss.mean()  # Reduced from 0.05 to 0.01 for better stability
        ...
        self.log('transmatcher_loss', transmatcher_loss.mean(), on_step=True, on_epoch=True, logger=True, prog_bar=True)  # Need .mean()
        self.log('transmatcher_acc', transmatcher_acc.mean(), on_step=True, on_epoch=True, logger=True)  # Need .mean()
        
        # Log to wandb
        wandb.log({
            "adaface_loss": adaface_loss.item(),
            "transmatcher_loss": transmatcher_loss.mean().item(),  # Need .mean()
            "transmatcher_acc": transmatcher_acc.mean().item(),  # Need .mean()
        ...
        for name, param in self.named_parameters():
            if param.grad is not None and torch.isnan(param.grad).any():
                print(f"[TRAIN_STEP] WARNING: NaN gradients detected in {name} and zeroed out")
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.SGD([
            ...
            {'params': transmatcher_params, 'lr': self.hparams.lr * 0.1, 'weight_decay': 5e-4},
