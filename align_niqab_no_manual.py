#!/usr/bin/env python3
"""
Niqab Alignment Without Manual Cropping
======================================

This script tries multiple strategies to detect faces in niqab images
without falling back to manual cropping. It focuses on improving
face detection rather than guessing.
"""
...
def try_niqab_detection_strategies(mtcnn_model, img):
    """
    Try multiple detection strategies specifically for niqab images
    """
    strategies = [
        # Strategy 1: Original image
        ("Original", lambda x: mtcnn_model.align(x)),
        
        # Strategy 2: Enhanced contrast and sharpness
        ...
        ("Larger", lambda x: mtcnn_model.align(x.resize((int(x.width*1.3), int(x.height*1.3)), Image.LANCZOS))),
        
        # Strategy 4: Enhanced + larger
        ("Enhanced+Larger", lambda x: mtcnn_model.align(preprocess_for_niqab_detection(x).resize((int(x.width*1.3), int(x.height*1.3)), Image.LANCZOS))),
        
        # Strategy 5: Try different crop regions (upper portion)
        ("UpperCrop", lambda x: mtcnn_model.align(x.crop((0, 0, x.width, int(x.height*0.8))))),
        
        # Strategy 6: Enhanced + upper crop
        ("Enhanced+UpperCrop", lambda x: mtcnn_model.align(preprocess_for_niqab_detection(x).crop((0, 0, x.width, int(x.height*0.8))))),
    ]
...
def align_niqab_no_manual(input_dir, output_dir, crop_size=(112, 112), device='cuda:0'):
    """
    Align niqab images without manual cropping fallback
    """
    ...
    mtcnn_model.min_face_size = 6  # Very small minimum face size
    ...
    for img_path in tqdm(image_files, desc="Aligning faces (no manual crop)"):
        try:
            # Load image
            img = Image.open(img_path).convert('RGB')
            
            # Try multiple detection strategies
            aligned_face, strategy_used = try_niqab_detection_strategies(mtcnn_model, img)
    ...
    print(f"\nAlignment complete (no manual cropping)!")
...
def main():
    parser = argparse.ArgumentParser(description='Align niqab images without manual cropping')
    ...
    print("Using improved MTCNN detection without manual cropping fallback")
    
    align_niqab_no_manual(
        args.input_dir, 
        args.output_dir, 
