...
class DataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        print("DEBUG: Starting train_dataloader")
        # Use RandomIdentitySampler for PK batching
        K = 4  # number of instances per identity
        P = self.batch_size // K  # number of identities per batch
        train_sampler = RandomIdentitySampler(self.train_dataset, num_instances=K)

        return DataLoader(self.train_dataset,
                          batch_size=P * K,  # total batch size must be P*K
