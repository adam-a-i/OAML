...
class GraphSampler(Sampler):
    def __init__(self, data_source, img_path, transformer, model, matcher, batch_size=64, num_instance=4, 
        ...
        # Build index dictionary silently
        self.index_dic = defaultdict(list)
        for index, (_, pid, _, _) in enumerate(data_source):
            self.index_dic[pid].append(index)
        
        if verbose:
            print(f"\nGraph Sampler initialization:")
            print(f"Total PIDs: {len(self.index_dic)}")
            print(f"Min/Max/Avg images per PID: {min(len(v) for v in self.index_dic.values())}/{max(len(v) for v in self.index_dic.values())}/{sum(len(v) for v in self.index_dic.values())/len(self.index_dic):.1f}")
        
        self.pids = list(self.index_dic.keys())
        self.num_pids = len(self.pids)

        # Initialize pointers for each PID
        self.sam_pointer = {}
        for pid in self.pids:
            self.sam_pointer[pid] = 0
    ...
    def graph_index(self):
        ...
        valid_pids = []
        for pid in self.pids:
            if len(self.index_dic[pid]) >= self.num_instance:
                valid_pids.append(pid)
        
        print(f"Found {len(valid_pids)} valid identities with {self.num_instance}+ images")
        ...
        with torch.no_grad():
            num_valid = len(valid_pids)
            if num_valid == 0:
                raise RuntimeError(f"No identities found with {self.num_instance}+ images")
            
            dist = dist + torch.eye(num_valid, device=dist.device) * 1e15
            topk = max(1, min(self.batch_size // self.num_instance - 1, num_valid - 1))
        ...
        for i, pid in enumerate(valid_pids):
            ...
            index = []
            for j in id_index:
                curr_pid = valid_pids[j]
                img_index = self.index_dic[curr_pid]
                len_p = len(img_index)
                index_p = []
                remain = self.num_instance
                while remain > 0:
                    end = min(self.sam_pointer[curr_pid] + remain, len_p)
                    idx = img_index[self.sam_pointer[curr_pid] : end]
                    index_p.extend(idx)
                    remain -= len(idx)
                    self.sam_pointer[curr_pid] = end
                    if end >= len_p:
                        shuffle(img_index)
                        self.sam_pointer[curr_pid] = 0
