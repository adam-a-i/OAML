...
from .evaluation_metrics.ranking import evaluate_lfw
...
class Evaluator(object):
    ...
    def evaluate(self, dataset, query_loader, gallery_loader):
        query = dataset.query
        ...
        if isinstance(dataset, LFW):
            rank1, mAP = evaluate_lfw(dist, query=query, gallery=gallery)
        else:
            rank1, mAP = evaluate_all(dist, query=query, gallery=gallery)

        if dataset.has_time_info:
            num_prob = len(query)
            num_gal = len(gallery)
        ...
        return rank1, mAP, rank1_rerank, mAP_rerank, rank1_tlift, mAP_tlift, dist.numpy(), dist_rerank, dist_tlift, pre_tlift_dict
