...
class QAConv(Module):
    def __init__(self, num_features, height, width, num_classes):
        """
        Inputs:
        ...
        self.class_embed = nn.Parameter(torch.randn(num_classes, num_features, height, width) / num_features**0.5)
    ...
    def forward(self, prob_fea, gal_fea=None):
        if self.training:
            if gal_fea is None:
                gal_fea = self.class_embed
            elif gal_fea == "same":
                gal_fea = prob_fea
            else:
                raise ValueError('gal_fea should be None or "same" in training mode')
